		...
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					...
					<el-col :span="24">
						<el-form-item label="布置时间" prop="buzhishijian">
							<el-date-picker
								class="list_date"
								v-model="form.buzhishijian"
								format="YYYY-MM-DD HH:mm:ss"
								value-format="YYYY-MM-DD HH:mm:ss"
								type="datetime"
								:readonly="!isAdd||disabledForm.buzhishijian?true:false"
								placeholder="请选择布置时间" />
						</el-form-item>
					</el-col>
					<el-col :span="24">
					...
					<el-col :span="24">
						<el-form-item label="作业状态" prop="zuoyezhuangtai">
							<el-select
								class="list_sel"
								:disabled="!isAdd||disabledForm.zuoyezhuangtai?true:false"
								v-model="form.zuoyezhuangtai" 
								placeholder="请选择作业状态"
								>
								<el-option v-for="(item,index) in zuoyezhuangtaiLists" :label="item"
									:value="item"
									>
								</el-option>
							</el-select>
...
<script setup>
	...
	const getInfo = ()=>{
		context?.$http({
			...
			res.data.data.zuoyeneirong = res.data.data.zuoyeneirong?(res.data.data.zuoyeneirong.replace(reg,'../../../cl029873/file')):'';
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		resetForm()
			form.value.buzhishijian = context?.$toolUtil.getCurDateTime()
		if(formId){
			id.value = formId
			type.value = formType
		}
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='buzhishijian'){
					form.value.buzhishijian = row[x];
					disabledForm.value.buzhishijian = true;
					continue;
				}
				if(x=='jiaoshigonghao'){
				...
				if(x=='zuoyezhuangtai'){
					form.value.zuoyezhuangtai = row[x];
					disabledForm.value.zuoyezhuangtai = true;
					continue;
				}
				if(x=='reversetime'){
		...
		//由上级字段带出不可改
		disabledForm.value.xingming = true;
		zuoyezhuangtaiLists.value = "已完成,未完成".split(',')
	}
	//初始化
	//声明父级调用
	defineExpose({
