					...
					<div class="search_view">
						...
						<div class="search_box">
							<el-select
								class="search_sel"
								clearable
								v-model="searchQuery.zuoyezhuangtai" 
								placeholder="作业状态"
								>
								<el-option v-for="item in zuoyezhuangtaiLists" :label="item" :value="item"></el-option>
							</el-select>
						</div>
					</div>
			...
			<el-table
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="center">
					<template #default="scope">
						...
						<el-button v-if="btnAuth('zuoyexinxi','作业提交')" type="success" @click="zuoyetijiaoCrossAddOrUpdateHandler(scope.row,'cross','','zuoyezhuangtai','已完成','已完成,未完成'.split(',')[0])">作业提交</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const getList = () => {
		...
		if(searchQuery.value.zuoyezhuangtai&&searchQuery.value.zuoyezhuangtai!=''){
			params['zuoyezhuangtai'] = searchQuery.value.zuoyezhuangtai
		}
		context?.$http({
			url: `${tableName}/page`,
			method: 'get',
	...
	const getzuoyezhuangtaiLists = () => {
		zuoyezhuangtaiLists.value = '已完成,未完成'.split(',')
	}
	getzuoyezhuangtaiLists()
	const searchClick = () => {
    ...
    const zuoyetijiaoCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			zuoyetijiaoFormModelRef.value.init(row.id,'cross','作业提交',row,'zuoyexinxi',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
