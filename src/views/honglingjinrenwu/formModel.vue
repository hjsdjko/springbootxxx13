		...
		<el-dialog v-model="formVisible" :title="formTitle" width="80%" destroy-on-close :fullscreen='false'>
			<el-form class="formModel_form" ref="formRef" :model="form" label-width="$template2.back.add.form.base.labelWidth" :rules="rules">
				<el-row>
					<el-col :span="24">
						<el-form-item label="任务名称" prop="renwumingcheng">
							<el-input class="list_inp" v-model="form.renwumingcheng" placeholder="任务名称"
								type="text"
								:readonly="!isAdd||disabledForm.renwumingcheng?true:false" />
						</el-form-item>
					</el-col>
					...
					<el-col :span="24">
						<el-form-item label="资源视频" prop="ziyuanshipin">
							<uploads
								:disabled="!isAdd||disabledForm.ziyuanshipin?true:false"
								type="file"
								action="file/upload" 
								tip="请上传资源视频" 
								:limit="1" 
								style="width: 100%;text-align: left;"
								:fileUrls="form.ziyuanshipin?form.ziyuanshipin:''" 
								@change="ziyuanshipinUploadSuccess">
							</uploads>
						</el-form-item>
					</el-col>
					<el-col :span="24">
						<el-form-item label="资源文档" prop="ziyuanwendang">
							<uploads
								:disabled="!isAdd||disabledForm.ziyuanwendang?true:false"
								type="file"
								action="file/upload" 
								tip="请上传资源文档" 
								:limit="1" 
								style="width: 100%;text-align: left;"
								:fileUrls="form.ziyuanwendang?form.ziyuanwendang:''" 
								@change="ziyuanwendangUploadSuccess">
							</uploads>
						</el-form-item>
					</el-col>
					<el-col :span="24">
					...
					<el-col :span="24">
						<el-form-item label="打卡任务" prop="dakarenwu">
							<el-input v-model="form.dakarenwu" placeholder="打卡任务" type="textarea"
							:readonly="!isAdd||disabledForm.dakarenwu?true:false"
							/>
						</el-form-item>
					</el-col>
				</el-row>
...
<script setup>
	...
	//资源视频上传回调
	const ziyuanshipinUploadSuccess=(e)=>{
		form.value.ziyuanshipin = e
	}
	//资源文档上传回调
	const ziyuanwendangUploadSuccess=(e)=>{
		form.value.ziyuanwendang = e
	}
	//methods
	...
	const init=(formId=null,formType='add',formNames='',row=null,table=null,statusColumnName=null,tips=null,statusColumnValue=null)=>{
		...
		if(formType == 'add'){
			...
			for(let x in row){
				...
				if(x=='dakarenwu'){
					form.value.dakarenwu = row[x];
					disabledForm.value.dakarenwu = true;
					continue;
				}
				if(x=='ziyuanshipin'){
					form.value.ziyuanshipin = row[x];
					disabledForm.value.ziyuanshipin = true;
					continue;
				}
				if(x=='ziyuanwendang'){
					form.value.ziyuanwendang = row[x];
					disabledForm.value.ziyuanwendang = true;
					continue;
				}
				if(x=='fabushijian'){
	...
	const save=()=>{
		...
		if(form.value.ziyuanshipin!=null) {
			form.value.ziyuanshipin = form.value.ziyuanshipin.replace(new RegExp(context?.$config.url,"g"),"");
		}
		if(form.value.ziyuanwendang!=null) {
			form.value.ziyuanwendang = form.value.ziyuanwendang.replace(new RegExp(context?.$config.url,"g"),"");
		}
