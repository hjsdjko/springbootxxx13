	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				<el-form :model="searchQuery" class="search_form" >
					<div class="search_view">
						...
						<div class="search_box">
							<el-input class="search_inp" v-model="searchQuery.jilumingcheng" placeholder="记录名称"
								clearable>
							</el-input>
						</div>
					</div>
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('tuoguanjilu','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('tuoguanjilu','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('tuoguanjilu','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)" v-if="btnAuth('tuoguanjilu','删除')">删除</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="记录视频" :resizable='true' :sortable='true' align="left" header-align="center">
					<template #default="scope">
						<el-button v-if="scope.row.jilushipin" type="text" size="small" @click="preClick(scope.row.jilushipin)">预览</el-button>
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="center">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('tuoguanjilu','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
	...
	const getList = () => {
		...
		params['sort'] = 'id'
		params['order'] = 'desc'
		if(searchQuery.value.jilumingcheng&&searchQuery.value.jilumingcheng!=''){
			params['jilumingcheng'] = '%' + searchQuery.value.jilumingcheng + '%'
		}
		context?.$http({
			url: `${tableName}/page`,
			method: 'get',
